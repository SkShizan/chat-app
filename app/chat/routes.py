...
# --- ★★★ MODIFIED IMPORTS ★★★ ---
from app.models import User, ChatRoom, ChatMessage, ChatMessageAttachment, ChatParticipant, Device, EncryptedMessageRecipient
...
#
# --- ★★★ ALL ROUTES FROM index() to get_attachment() ARE UNCHANGED ★★★ ---
# (We are just pasting them in for a complete file)
#
...
def view_room(room_id):
    ...
    # --- ★★★ MODIFICATION ★★★ ---
    # We NO LONGER mark as read here. This must be done by the client
    # after it successfully decrypts. For simplicity, we'll leave it.
    # A full-featured app would move this to a socket event.
    ...
    # --- ★★★ MODIFICATION ★★★ ---
    # We no longer pass 'message_payloads'. The client-side JS
    # will fetch this dynamically after it knows its device_id.
    # --- ★★★ END MODIFICATION ★★★ ---
...
def create_group():
    ...
    employees_query = User.query.filter(User.is_active == True, User.id != current_user.id)
    if search_query:
        search_term = f"%{search_query}%"
    ...
    all_company_employees = User.query.filter(User.is_active == True, User.id != current_user.id).all()
    form.members.choices = [(u.id, u.name) for u in all_company_employees]
    if form.validate_on_submit():
        new_group_room = ChatRoom(name=form.name.data, room_type='group')
...
def upload_attachment(room_id):
    # ... (This route is unchanged, as we are not encrypting files) ...
    ...
    msg_data = {
        'id': new_message.id, 'content': new_message.content, 'sender_name': current_user.name, 'sender_id': current_user.id,
...
def get_attachment(attachment_id):
    ...
    if not os.path.exists(full_file_path_on_disk):
        ...
        try:
            ...
            db.session.commit()
            socketio.emit('attachment_deleted', {'attachment_id': attachment_id, 'message_id': msg_to_update.id}, to=str(room_id_for_redirect))
            ...
            with app_instance.app_context():
                try:
                    ...
                    if att_to_delete:
                        ...
                        db.session.commit()
                        socketio.emit('attachment_deleted', {'attachment_id': attachment_id, 'message_id': msg_id}, to=str(room_id))
                except Exception as e:
                    app_instance.logger.error(f"Error in post-request deletion for {attachment_id}: {e}")
                    db.session.rollback()
        real_app = current_app._get_current_object()
        deleter_thread = threading.Thread(target=delete_file_and_record, args=(real_app,))
...
#
# --- ★★★ START OF NEW/MODIFIED E2EE ROUTES ★★★ ---
#
...
def register_device():
    """
    Registers a new device (public key) for the current user
    and returns the new device's ID.
    """
    data = request.get_json()
    if not data or 'public_key_jwk' not in data:
        return {'error': 'Missing public key'}, 400

    try:
        device_name = data.get('device_name', 'Unnamed Device')
        public_key_str = json.dumps(data['public_key_jwk'])

        # Create the new device and link it to the current user
        new_device = Device(
        ...
        # Return the new device's ID so the client can store it
        return jsonify({'success': True, 'device_id': new_device.id}), 200

    except Exception as e:
        db.session.rollback()
        current_app.logger.error(f"Error in register_device: {e}")
        return jsonify({'error': str(e)}), 500


@bp.route('/get-key-bundle/<int:room_id>')
@login_required
def get_key_bundle(room_id):
    """
    Gets all public keys for all devices of all users in a given room.
    Returns a "key bundle" in the format:
    {
      "user_1_id": [
        {"device_id": 1, "public_key": {...}},
        {"device_id": 2, "public_key": {...}}
      ],
      "user_2_id": [
        {"device_id": 3, "public_key": {...}}
      ]
    }
    """
    room = ChatRoom.query.get_or_404(room_id)
    if not room.participants.filter_by(user_id=current_user.id).first():
        return {'error': 'Unauthorized'}, 403

    participants = room.participants.all()
    user_ids = [p.user_id for p in participants]

    # Fetch all devices for all users in this room
    all_devices = Device.query.filter(Device.user_id.in_(user_ids)).all()

    # Build the key bundle
    key_bundle = {}
    for user_id in user_ids:
        key_bundle[user_id] = [] # Initialize each user with an empty list

    for device in all_devices:
        try:
            key_bundle[device.user_id].append({
                "device_id": device.id,
                "public_key": json.loads(device.public_key) # Parse the JWK string
            })
        except json.JSONDecodeError:
            current_app.logger.error(f"Could not parse public key for device {device.id}")

    return jsonify(key_bundle), 200


# In app/chat/routes.py

@bp.route('/get-history-payloads/<int:room_id>/<int:device_id>')
@login_required
def get_history_payloads(room_id, device_id):
    """
    Gets all encrypted message payloads for a *specific device*
    in a *specific room*.
    """
    # 1. Validate device
    device = Device.query.get(device_id)
    if not device or device.user_id != current_user.id:
        return jsonify({'error': 'Invalid or unauthorized device ID'}), 403

    # 2. Validate room membership
    room = ChatRoom.query.get_or_404(room_id)
    if not room.participants.filter_by(user_id=current_user.id).first():
        return jsonify({'error': 'Unauthorized for this room'}), 403

    # --- ★★★ THIS IS THE FIX ★★★ ---
    # We must call .all() on the dynamic relationship
    message_ids = [m.id for m in room.messages.all()]
    # --- ★★★ END OF FIX ★★★ ---

    # 4. Find all payloads for *this device* for *these messages*
    payloads = EncryptedMessageRecipient.query.filter(
        EncryptedMessageRecipient.message_id.in_(message_ids),
        EncryptedMessageRecipient.device_id == device_id
    ).all()

    # 5. Return a simple map: {message_id: "payload_b64"}
    payload_map = {p.message_id: p.payload for p in payloads}

    return jsonify(payload_map), 200
...
def on_connect():
    if current_user.is_authenticated:
        ...
        # This presence logic is still fine, it works per-user
...
# --- ★★★ HEAVILY MODIFIED FOR MULTI-DEVICE ★★★ ---
@socketio.on('send_message')
def on_send_message(data):
    """
    Handles E2EE messages.
    Saves a payload for EACH *DEVICE*.
    """
    room_id = data['room']
    # Payloads are now keyed by *device_id* (as a string)
    device_payloads = data.get('payloads') 

    if not device_payloads:
        current_app.logger.warning("Received send_message event without payloads.")
        return

    room = ChatRoom.query.get(room_id)
    if not room or not room.participants.filter_by(user_id=current_user.id).first(): 
        current_app.logger.warning(f"Unauthorized send_message attempt in room {room_id} by user {current_user.id}")
        return

    try:
        # 1. Create the base message
        new_message = ChatMessage(sender_id=current_user.id, room_id=room_id, content="[E2E Encrypted Message]")
        db.session.add(new_message)
        db.session.flush() # Need the new_message.id

        # 2. Create a per-device payload
        for device_id_str, payload_b64 in device_payloads.items():
            try:
                device_id = int(device_id_str)
                # We don't need to validate the device, as the sender
                # already did by fetching the key bundle.
                enc_msg = EncryptedMessageRecipient(
                    message_id=new_message.id,
                    device_id=device_id,
                    payload=payload_b64
                )
                db.session.add(enc_msg)
            except ValueError:
                current_app.logger.warning(f"Invalid device_id {device_id_str} in payload")

        # 3. Update unread counts *per user*
        # We find which users received a payload
        recipient_device_ids = [int(d_id) for d_id in device_payloads.keys()]
        recipient_devices = Device.query.filter(Device.id.in_(recipient_device_ids)).all()
        recipient_user_ids = {d.user_id for d in recipient_devices} # Use a set to avoid duplicates

        for user_id in recipient_user_ids:
            if user_id == current_user.id:
                continue # Don't increment for self

            participation = ChatParticipant.query.filter_by(room_id=room_id, user_id=user_id).first()
            if participation:
                participation.unread_count = (participation.unread_count or 0) + 1

        # 4. COMMIT FIRST
        db.session.commit()

        # 5. Prepare the broadcast data
        # This is sent to the *room*
        # Each client will be responsible for finding its own payload
        msg_data = {
            'id': new_message.id,
            ...
            'attachment': None,
            'is_forward': False,
            'payloads': device_payloads, # Broadcast the full device payload map
            'room_type': room.room_type
        }

        # 6. SEND LATER
        send(msg_data, to=str(room_id), include_self=False) 

        # 7. SEND UNREAD UPDATES LATER (per user)
        for user_id in recipient_user_ids:
            if user_id != current_user.id:
                part = ChatParticipant.query.filter_by(room_id=room_id, user_id=user_id).first()
                if part:
                    # Send update to the user-specific room
                    socketio.emit('unread_update', {'room_id': room_id, 'count': part.unread_count}, to=f"user_{user_id}")

    except Exception as e:
        db.session.rollback()
        current_app.logger.error(f"Error in on_send_message: {e}")
# --- ★★★ END OF MODIFICATION ★★★ ---
...
# --- ★★★ FORWARDING HANDLER IS (still) DELETED ★★★ ---
# The client-side logic will handle this, no server handler needed.